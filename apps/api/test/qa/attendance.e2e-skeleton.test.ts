...
test('QA-05 设备签到链：register device -> bind student -> attendance event -> homework time -> daily stat executable', async (t) => {
  ...
  await t.test('smoke: register/bind/checkin/session/stat path is executable', () => {
    const device = attendanceService.createDevice({
      campusId: 'campus-001',
      serialNo: 'QA-BEACON-001',
      deviceType: 'beacon',
      note: 'qa attendance path',
    });
    const binding = attendanceService.createBinding({
      studentId: 'student-qa-001',
      deviceId: device.id,
      createdBy: 'user-admin-001',
      boundAt: '2026-03-25T07:59:00+08:00',
    });
    const event = attendanceService.createEvent({
      studentId: 'student-qa-001',
      campusId: 'campus-001',
      deviceId: device.id,
      eventType: 'checkin',
    ...
    const session = attendanceService.createHomeworkTimeSession({
      studentId: 'student-qa-001',
      campusId: 'campus-001',
    ...
    const stats = attendanceService.getHomeworkTimeDailyStats({
      studentId: 'student-qa-001',
      dateFrom: '2026-03-25',
      dateTo: '2026-03-25',
      pageNo: 1,
  ...
  await t.test('case-idempotent-event-and-query-filters-are-executable', () => {
    const device = attendanceService.createDevice({
      campusId: 'campus-001',
      serialNo: 'QA-BEACON-002',
      deviceType: 'beacon',
    });
    attendanceService.createBinding({
    ...
    const first = attendanceService.createEvent({
      studentId: 'student-002',
      campusId: 'campus-001',
      deviceId: device.id,
      eventType: 'checkin',
    ...
    const replay = attendanceService.createEvent({
      studentId: 'student-002',
      campusId: 'campus-001',
      deviceId: device.id,
      eventType: 'checkin',
    ...
    const queried = attendanceService.listEvents({
      studentId: 'student-002',
      eventType: 'checkin',
      dateFrom: '2026-03-26',
      dateTo: '2026-03-26',
  ...
  await t.test('case-binding-and-device-student-guardrails-are-enforced', () => {
    const device = attendanceService.createDevice({
      campusId: 'campus-001',
    ...
    assert.throws(() => attendanceService.createEvent({
    ...
    assert.throws(() => attendanceService.createHomeworkTimeSession({
      studentId: 'student-qa-004',
      campusId: 'campus-001',
      termId: 'term-2026-spring',
      subject: 'math',
