  ...
  return (
    <PermissionGuard allowed={allowed} fallback={<PermissionDeniedState resource="沟通记录" permissionCode={communicationPermissions.recordsView} />}>
      <div className="stack">
        ...
        <MetricGrid items={[{ label: '当前记录数', value: String(result.page.total), hint: '全部沟通记录' }, { label: '当前筛选首条主题', value: result.list[0]?.subject ?? '--', hint: '最近一条记录' }, { label: '高频渠道', value: topChannel, hint: '按当前页记录聚合' }]} />
        <section className="panel stack" id="communication-record-create-form">
          <div className="page-header"><div><h3>新建沟通记录</h3></div></div>
