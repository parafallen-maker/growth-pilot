import { DataTable, FilterBar, MetricGrid, PageHeader, PaginationBar, SummaryPanel, TimelinePanel } from '@/components/business/page-blocks';
...
export default async function CommunicationRecordsPage({
  ...
  const [result, families, students] = await Promise.all([
    communicationService.queryRecords(filters).catch(() => ({ list: [], page: { pageNo: 1, pageSize: 20, total: 0 } })),
  ...
  const detail = result.list[0] ? await communicationService.detailRecord(result.list[0].recordId).catch(() => ({ timeline: [], linkedActions: [], subject: '--' })) : { timeline: [], linkedActions: [], subject: '--' };
  const channelCount = result.list.reduce<Map<string, number>>((map, item) => { map.set(item.channel, (map.get(item.channel) ?? 0) + 1); return map; }, new Map());
  ...
  return (
    <PermissionGuard allowed={allowed} fallback={<PermissionDeniedState resource="沟通记录" permissionCode={communicationPermissions.recordsView} />}>
      <div className="stack">
        ...
        <PageHeader title="沟通记录" description="家校沟通记录管理" actions={<><a className="btn primary" href="#communication-record-create-form">新建沟通</a><button className="btn">关联会谈</button><CsvExportButton filename="communication-records.csv" headers={['时间', '家庭', '学生', '渠道', '方向', '主题', '记录人']} rows={result.list.map((item) => [item.occurredAt, item.familyName, item.studentName, item.channel, item.direction, item.subject, item.recorder])} /></>} />
        ...
        <MetricGrid items={[{ label: '当前记录数', value: String(result.page.total), hint: 'communication/records 持久化台账' }, { label: '当前筛选首条主题', value: result.list[0]?.subject ?? '--', hint: '来自真接口首条记录' }, { label: '高频渠道', value: topChannel, hint: '按当前页真实记录聚合' }, { label: '后端缺口', value: 'meeting / 家庭任务聚合未开放', hint: '详情页先展示 nextAction，不伪造反查闭环' }]} />
        <section className="panel stack" id="communication-record-create-form">
          <div className="page-header"><div><h3>新建沟通记录</h3><p>当前表单直连 POST /communication/records；meeting / 家庭任务反查缺口继续明确展示，不伪造闭环。</p></div><span className="badge success">POST /communication/records</span></div>
