  ...
  return (
    <PermissionGuard allowed={allowed} fallback={<PermissionDeniedState resource="消息中心" permissionCode={communicationPermissions.messagesView} />}>
      <div className="stack">
        ...
        <MetricGrid items={[
          { label: '模板数量', value: String(result.templates.page.total),  },
          { label: '草稿箱', value: String(result.drafts.page.total) },
          { label: '待发送', value: String(result.queued.page.total) },
          { label: '失败 / 回执', value: `${result.failed.page.total} / ${result.sent.list.filter((item) => item.status === 'read').length}`,  },
        ]} />
        ...
        <SummaryPanel title="状态说明" items={result.statusPanels} />
      </div>
    </PermissionGuard>
  );
}
