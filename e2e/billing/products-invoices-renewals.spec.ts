...
test.describe('E2E-019~021 收费三页', () => {
  ...
  test('账单列表加载+欠费预警 (E2E-020)', async ({ page }) => {
    ...
    // IMPORTANT: assertion timeout must be long enough for server to complete
    // all 6 parallel API calls. Short timeout = test passes early but server
    // is still processing → next test's login gets blocked.
    await expect(page.getByRole('table').first()).toBeVisible({ timeout: 15000 }).catch(() => {
      return expect(page.getByText(/账单|支付|Invoice/).first()).toBeVisible({ timeout: 5000 });
    });
  ...
  test('续费跟进页面加载 (E2E-021)', async ({ page }) => {
    ...
    // Don't wait for networkidle — rely on assertion timeout instead
