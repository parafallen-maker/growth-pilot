...
### A10-1 migration 真导入
- [x] 接真实 Excel/CSV parser（当前已支持 repo 内 JSON/CSV 样本输入）
- [ ] 接 PostgreSQL staging schema
- [ ] 接 final load upsert
- [x] 第一批真实样本导入（当前为 repo 示例样本批次）
...
### A10-2 QA 真联调
- [x] skeleton tests 扩成真实 E2E（当前先升级 homework / growth 两条 smoke）
